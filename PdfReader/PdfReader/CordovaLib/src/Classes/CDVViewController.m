...
- (void)viewDidLoad
{
    ...
    if ([self.startPage rangeOfString:@"://"].location != NSNotFound) {
        ...
        if ([[self.wwwFolderName lowercaseString] hasPrefix:@"file://"]) {
            self.wwwFolderName = [self.wwwFolderName substringFromIndex:7];
            NSURL *url = [NSURL fileURLWithPath:self.wwwFolderName];
            self.wwwFolderName = [url absoluteString];
        }
